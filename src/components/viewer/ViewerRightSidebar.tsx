...
import {
    ...
    Layers, PanelRightClose, PanelRightOpen, X
} from 'lucide-react';
...
const ViewerRightSidebar = ({
    ...
    return (
        <aside
            // [애니메이션 수정] 열림/닫힘 상태에 따라 너비 변경
            className={`
                bg-[#161616] border-l border-white/10 flex z-50 shadow-2xl relative shrink-0
                transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] 
                ${isOpen ? 'w-[460px]' : 'w-16'}
            `}
        >
            {/* --- [A] Main Content Area --- */}
            {/* [애니메이션 수정] w-[396px] 고정 너비를 주어 텍스트 찌그러짐 방지 */}
            <div className={`
                flex flex-col h-full overflow-hidden 
                transition-all duration-300 ease-in-out
                ${isOpen ? 'w-[396px] opacity-100' : 'w-0 opacity-0 pointer-events-none'}
            `}>
                    ...
                    <div className="h-14 flex items-center justify-between px-6 border-b border-white/5 shrink-0">
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="cursor-pointer text-gray-500 hover:text-white transition-colors p-1 rounded hover:bg-white/5"
                                title="패널 닫기"
                            >
            ...
            <div className="flex flex-col gap-4 w-16 py-6 items-center border-l border-white/10 bg-[#1a1a1a] shrink-0 z-10">
                ...
                {/* 닫혔을 때만 보이는 '열기' 버튼 */}
                <div className={`transition-all duration-300 ${!isOpen ? 'opacity-100 h-auto mb-4' : 'opacity-0 h-0 overflow-hidden'}`}>
                    <button
                        onClick={() => setIsOpen(true)}
                        // [수정] cursor-pointer 추가
                        className="cursor-pointer p-3 rounded-xl bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white transition-all"
                        title="패널 열기"
                    >
